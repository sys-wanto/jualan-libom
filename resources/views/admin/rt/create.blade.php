<x-app-layout>
    ...
    <div class="py-12">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="">
                ...
                @endif
                <form class="w-full" action="{{ route('admin.rt.store') }}" method="post"
                    ...
                    <div class="flex flex-wrap px-3 mt-4 mb-6 -mx-3">
                        <div class="w-full">
                            ...
                            onchange="getkelurahan($('#kd_propinsi').val(),$('#kd_dati2').val(),this.value)"
                                class="text-base w-full font-medium focus:border-primary focus:outline-none placeholder:text-secondary placeholder:font-normal px-[26px] py-4 border border-grey rounded-[50px] focus:before:appearance-none focus:before:!content-none"
                    ...
                    <div class="flex flex-wrap px-3 mt-4 mb-6 -mx-3">
                        <div class="w-full">
                            ...
                            <select name="kd_kelurahan" id="kd_kelurahan"
                                class="text-base w-full font-medium focus:border-primary focus:outline-none placeholder:text-secondary placeholder:font-normal px-[26px] py-4 border border-grey rounded-[50px] focus:before:appearance-none focus:before:!content-none"
                                placeholder="Pilih Kelurahan" require>
                                @foreach ($kelurahan as $p)
                                    <option value="{{ $p->kd_kelurahan_new }}">{{ $p->kd_kelurahan_new }} -
                                        {{ $p->nm_propinsi }} </option>
                                @endforeach
                            </select>
                            <div class="mt-2 text-sm text-gray-500">
                    ...
                    <div class="flex flex-wrap px-3 mt-4 mb-6 -mx-3">
                        <div class="w-full">
                            ...
                            <input value="{{ old('kd_rt') }}" name="kd_rt" maxlength="3"
                    ...
                    <div class="flex flex-wrap px-3 mt-4 mb-6 -mx-3">
                        <div class="w-full">
                            ...
                            <input value="{{ old('nm_rt') }}" name="nm_rt"
                                class="block w-full px-4 py-3 leading-tight text-gray-700 bg-gray-200 border border-gray-200 rounded appearance-none focus:outline-none focus:bg-white focus:border-gray-500"
    ...
    <x-slot name="script">
        <script>
            ...
            function getkelurahan(kd_propinsi = '', kd_dati2 = '',kd_kecamatan='') {
                $.ajax({
                    url: "{{ url('/') }}/api/get/kelurahan/" + kd_propinsi + "/" + kd_dati2+ "/" + kd_kecamatan,
                    method: "GET",
                    success: function(response) {
                        ...
                        if (response.length > 0) {
                            response.forEach(value => {
                                console.log(value);
                                option += "<option value='" + value.kd_kelurahan_new + "''>" + value
                                    .kd_kelurahan_new + " - " + value.nm_kelurahan + " </option>"
                            });
                        } 
